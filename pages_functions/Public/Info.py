...
class Info(QDialog):
    ...
    def Add_order(self,type,id,name,action,message):
        if     type == 0 :  type = "❌"
        elif   type == 1 :  type = "✅"
        else : type = "⚠️"
        ...
        self.ui.table.setItem(0, 4 , QTableWidgetItem(action)) 
        self.ui.table.setItem(0, 5 , QTableWidgetItem(message))
        try: cursor.execute('INSERT INTO Report_order (Type, Id , Data , Name, Acction ,Message) VALUES ( ?, ?, ?, ?, ?, ?)', ( type , id , data, name, action, message)) ;conn.commit()
        except Exception as e : print("Faield Update",e)
