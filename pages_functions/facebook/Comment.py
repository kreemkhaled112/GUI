...
from ui.Facebook.Comment_ui import Ui_Form
from pages_functions.Public.Info import Info
from pages_functions.Facebook.Data.get_count import *
from pages_functions.Facebook.Data.Action import Comment as comment
...
class Comment(QWidget):
    def __init__(self):
        ...
        self.ui.pushButton.clicked.connect(lambda: (config.set('server', 'Comment', self.ui.lineEdit.text()), config.write(open('pages_functions\settings.ini', 'w')),self.ui.label.setText(config['server']['comment']))) ; self.ui.label.setText(config['server']['comment'])
        ...
        if self.ui.Number_Account.text() == '0': QMessage(text = 'No Account Selected').mainloop() ; self.ui.Start.setChecked(False)
            ...
            while  self.is_running :
                    ...
                    if pending["status"] == "success":
                        ...
                        comments  = queue.Queue(); [comments.put(i) for i in pending['comments'].split('\n')]
                        result = get_comment(link).Start()
                        listt = []
                        if result[0] == 'No match found' :
                            self.Info.Add_order(0,id,'None',"Comment",f'Error {result[1]}')
                                ...
                                while not self.queue.empty():
                                    if self.succes >= quantity : break
                                        ...
                                        try:
                                            ...
                                            self.Info.Add_order(result[1],id,cookie[1],"Comment",f'{result[0]} {link}')
