...
from pages_functions.Facebook.Data.Action import Share as share
...
class Share(QWidget):
    def __init__(self):
        ...
        self.ui.pushButton.clicked.connect(lambda: (config.set('server', 'Share', self.ui.lineEdit.text()), config.write(open('pages_functions\settings.ini', 'w')),self.ui.label.setText(config['server']['share']))) ; self.ui.label.setText(config['server']['share'])
        ...
        if self.ui.Number_Account.text() == '0': QMessage(text = 'No Account Selected').mainloop() ; self.ui.Start.setChecked(False)
            ...
            while  self.is_running :
                    ...
                    if pending["status"] == "success":
                        ...
                        result = get_share(link).Start()
                        listt = []
                        if result[0] == 'No match found' :
                            self.Info.Add_order(0,id,'None',"Share",f'Error {result[1]}')
                                ...
                                while not self.queue.empty() :
                                    if self.succes >= quantity : break
                                        ...
                                        try:
                                            ...
                                            listt.append(cookie)
                                            result = share(link,"",cookie[5]).Start()
                                            self.Info.Add_order(result[1],id,cookie[1],"Share",f'{result[0]} {link}')
                                            if result[1] == 1: self.succes += 1
                                            else: self.failed += 1
                                            self.Info.Update(s=self.succes,f=self.failed,o=self.order) ;  sleep(self.time)
                                        except Exception as e:
                                            try: self.Info.Add_order(0,id,cookie[1],"Share",e)
                                            except : self.Info.Add_order(0,id,'None',"Share","No Account") 
                                            self.failed += 1
                                            self.Info.Update(s=self.succes,f=self.failed,o=self.order) ;  sleep(self.time)
                               
                            except Exception as e : input(e)
                        self.order += 1
                        self.Info.Add_order(1,id,'Compelet',"Share",f'Total : {quantity} Succes : {self.succes} Failed : {self.failed} Link {link}',"ok") ; self.Info.Update(s=self.succes,f=self.failed,o=self.order) 
